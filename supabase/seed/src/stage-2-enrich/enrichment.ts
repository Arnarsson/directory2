...
import { batchToSnakeCase } from "../../../../lib/tag-label-utils"
...
const cheapFastModelConfig: AIClientConfig = {
  ...
  model: anthropicApiKey
    ? ("claude-3-5-haiku-20241022" as AIModel)
    : ("gpt-5-nano" as AIModel),
...
const smartModelConfig: AIClientConfig = {
  provider: anthropicApiKey ? "anthropic" : "openai",
  model: anthropicApiKey
    ? ("claude-sonnet-4-20250514" as AIModel)
    : ("gpt-5-mini" as AIModel),
  apiKey: anthropicApiKey || openaiApiKey!,
}
...
const enrichItem = async (
  ...
  return {
    ...
    tags: batchToSnakeCase(result.object.tags),
    labels: batchToSnakeCase(result.object.labels),
    categories: batchToSnakeCase([result.object.category])[0] || "undefined",
  }
}
...
const enrichItemWithSeparateRequests = async (
  ...
  try {
    ...
    // If validation fails, fix labels and tags (but now validation is more flexible)
    ...
    const combinedOutput = {
      ...
      tags: fixedLabelsTags.tags.slice(0, 4), // Take up to 4 tags
      labels: fixedLabelsTags.labels.slice(0, 3), // Take up to 3 labels
      category: filtersOutput.object.category || "undefined", // Ensure category is not undefined
    }

    // Validate the combined output (now more flexible)
    ...
    if (!strictValidation.success) {
      ...
      throw new Error(
        `Failed to validate attempt 1 output for ${
          item.codename
        }: ${JSON.stringify(strictValidation.error.issues, null, 2)}`
      )
    }
    ...
    return {
      ...item,
      ...combinedOutput,
      tags: batchToSnakeCase(combinedOutput.tags),
      labels: batchToSnakeCase(combinedOutput.labels),
      categories: batchToSnakeCase([combinedOutput.category])[0] || "undefined",
    }
  } catch (error) {
